...
pml_layers = [mp.PML(thickness=pml_size, direction = mp.Y),mp.Absorber(thickness=pml_size, direction = mp.X),]
...
# FOM 
def J(fields1):
    field1_integral = npa.sum(npa.abs(fields1) ** 2, axis=(1,2))  
...
cur_beta = 4
beta_scale = 2
update_factor = 14  # num. iteration per  epoch
num_betas = 15 # total epochs
ftol = 1e-5
...
def f(v, gradient, beta):    
    ...
    np.save(f"{data_path}/st_{eta_i}_{beta}_{cur_iter[0]+1:03d}.npy", v)
    if mp.am_master():
        plt.figure(figsize=(5, 5))
        ax = plt.gca()
        opt.plot2D(
...
def plot_final(design):
    ...
    opt.plot2D(
        ...
        plot_monitors_flag=False,
        plot_boundaries_flag=False,
        output_plane = mp.Volume(center=design_region.center, size=mp.Vector3(dx, dy, 0))
    )
