...
# === cavity ===
cavity_design = np.load("initial_structure.npy")
...
def f(v, gradient, beta):
    ...
    if gradient.size > 0:
        gradient[:] = tensor_jacobian_product(mapping, 0)(x, eta_i, beta,npa.sum(dJ_du,axis=1)) / nfreq
    ...
    np.save(f"{data_g}/{cur_iter[0]+1:03d}.npy", gradient.astype(np.float32))
